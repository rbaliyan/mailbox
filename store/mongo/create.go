...
func (s *Store) CreateMessageIdempotent(ctx context.Context, data store.MessageData, idempotencyKey string) (store.Message, bool, error) {
	...
	// Build the document to insert if not exists, stamping it with a candidate
	// _id generated by this call. With the unique index on
	// (owner_id, idempotency_key), exactly one concurrent caller's upsert
	// performs the insert, so exactly one caller's candidate _id ends up stored.
	// Comparing the returned _id to our candidate is therefore a reliable
	// single-winner signal, unlike comparing a locally-computed timestamp (which
	// races when concurrent callers happen to compute the same instant).
	...
	// This call inserted the document iff the stored _id is the one it generated.
	created := result.ID == candidateID
...
// that were actually deleted by this call.
//
// Each ID is removed with an atomic per-document DeleteOne, so exactly one
// caller observes DeletedCount == 1 for a given document even when many callers
// race on the same IDs. This single-winner reporting is what guards against
// double attachment-ref release: only the winner reports the ID as deleted.
func (s *Store) DeleteMessagesByIDs(ctx context.Context, ids []string) ([]string, error) {
	...
	for _, id := range ids {
		...
		if err != nil {
			// Unparseable IDs can never correspond to a stored document; skip
			// them just as an absent ID is skipped.
			continue
		}
		result, err := s.collection.DeleteOne(ctx, bson.M{"_id": oid})
		if err != nil {
			return deleted, fmt.Errorf("delete messages by IDs: %w", err)
		}
